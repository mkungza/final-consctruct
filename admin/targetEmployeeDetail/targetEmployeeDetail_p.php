...
function getTransaction($pageNow,$strSort,$strOrder,$userid){
	...
	if($userid!="")
	{
		$sql = "SELECT tranid,createdate,amount,tu.name as name, transname, transaddr,transtel ,ts.tstatusname as status FROM ttransact ta inner join tuser tu on tu.userid = ta.userid inner join ttranstatus ts on ts.tstatusid = ta.transtatus  where ta.userid='$userid'";
	}
	else{
		$sql = "SELECT tranid,createdate,amount,tu.name as name, transname, transaddr,transtel ,ts.tstatusname as status FROM ttransact ta inner join tuser tu on tu.userid = ta.userid inner join ttranstatus ts on ts.tstatusid = ta.transtatus  ";
	}	
